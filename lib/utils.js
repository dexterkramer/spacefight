...
function nextPlayer(game, rewind)
{
    if(game.turn.player == null)
    {
        if(typeof game.players[0] !== "undefined" && game.players[0] !== null)
        {
            game.turn.player = game.players[0];
        }        
        return game.turn.player;
    }
    if(typeof game.players[game.turn.player.number + 1] !== "undefined" && game.players[game.turn.player.number + 1] !== null)
    {
         game.turn.player = game.players[game.turn.player.number + 1];
    }
    else
    {
...
function drawPlayerSquads(player)
{
    ...
    let oneSquad = this.squadsGroup.create(x, y, 'squad');
    oneSquad.anchor.x = 0.5;
    oneSquad.anchor.y = 0.5;
    oneSquad.scale.setTo(squadWidth / oneSquad.width, squadHeight / oneSquad.height);
    oneSquad.ref = squad;
